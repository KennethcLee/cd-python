class BankAccount:
    ...
    def __init__(self, acctnum=0, int_rate = 0.01, balance=0):
    ...
    def display_account_info(self, acctnum=0):
        return self.balance
...
class User:
    ...
    def __init__(self, fname, lname, init_rate=0.01, amount=0):
        self.fname = fname
        self.lname = lname
        self.all_accounts += BankAccount(init_rate, amount)

    def displayUserBalance(self, acctnum):
        print(f"{self.fname} {self.lname} Account#{acctnum} has ${self.account.display_account_info(acctnum)}")
        return self
...
# peter = User("Peter", "Hong", 0, 0.02, 50)
# mary = User("Mary", "Lin", 0, 0.03, 100)
joe = User("Joe", "Chang", 0.01, 1)
joe = User("Joe", "Chang", 1, 0.03, 100)

# peter.displayUserBalance(acctnum=0)
joe.displayUserBalance(acctnum=0)
joe.displayUserBalance(acctnum=1)
